    ...
    public class RootDialog : IDialog<object>
    {
        ...
        private async Task MessageReceivedAsync(IDialogContext context, IAwaitable<object> result)
        {
            ...
            catch (Exception e)
            {
                ...
                await context.PostAsync(e.ToString()).ConfigureAwait(false);
            } 
        ...
        private async Task HandleActions(IDialogContext context, Activity activity)
        {
            ...
            switch (type)
            {
                case Constants.ShowDetailedRoster:
                    ...
                    break;
                case Constants.NextWeekRoster:
                    //string crewid = ActiveDirectoryHelper.GetCrewId("drissm");
                    card = await CardHelper.GetWeeklyRosterCard("10055"); 
                   // card = await CardHelper.GetWeeklyRosterCard("userDetails.UserPrincipalName");
                    break;
                case Constants.NextMonthRoster:
                    card = CardHelper.GetMonthlyRosterCard();
        ...
        private async Task<TeamsChannelAccount> GetCurrentUserDetails(Activity activity)
        {
            ...
            try
            {
                ...
                //var creditionals = new MicrosoftAppCredentials("07edee8f-64ed-442f-93fa-6c54fda32a35", "_6-ltY5FHS~1bqiPqPzLibT5_iV-2-Qsv4");
                var creditionals = new MicrosoftAppCredentials(Utils.GetAppSetting("MicrosoftAppId"), Utils.GetAppSetting("MicrosoftAppPassword"));
                 ConnectorClient connector = new ConnectorClient(new Uri(activity.ServiceUrl),creditionals);
